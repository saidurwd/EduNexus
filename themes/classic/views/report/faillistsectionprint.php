<?php
$this->pageTitle = 'Fail List - Section Wise';
...
<h2 style="text-transform: uppercase; text-align: center; background-color: #DDD; padding: 5px 0px;">Fail List - Section Wise</h2>
<table class="table">
    <thead>
        <tr>
            <th>Class</th>
...
<table class="table" style="margin-top: 10px;">
    ...
    <tbody>
        ...
        $command = $connection->createCommand('SELECT SG.`title`, M.`total_mark`, M.`failed_subject`, S.`name`, M.`sid`, M.`roll`
                                                FROM {{merit_position}} M
                                                LEFT OUTER JOIN {{student}} S ON S.`id`=M.`student` 
                                                LEFT OUTER JOIN {{student_group}} SG ON SG.`id`= M.`group` 
                                                WHERE  M.`institution`=' . Yii::app()->user->institution . ' AND M.`class`=' . $class . ' AND M.`section`=' . $section . ' AND M.`exam`=' . $exam . ' AND M.`academic_year`=' . $year . ' AND M.`failed_subject`!=0 ORDER BY M.`failed_subject` ASC, M.`total_mark` DESC, M.`roll` ASC');
