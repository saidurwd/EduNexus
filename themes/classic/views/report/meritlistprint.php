...
<h2 style="text-transform: uppercase; text-align: center; background-color: #DDD; padding: 5px 0px;">Merit List - Group Wise</h2>
<table class="table">
    <thead>
        <tr>
            <th>Class</th>
...
<table class="table" style="margin-top: 10px;">
    ...
    <tbody>
        ...
        $command = $connection->createCommand('SELECT M.`total_mark`, M.`letter_grade`, M.`gpa`, M.`group_position`, S.`name`, M.`sid`, M.`roll`
                                                FROM {{merit_position}} M
                                                LEFT OUTER JOIN {{student}} S ON S.`id`=M.`student` 
                                                WHERE  M.`institution`=' . Yii::app()->user->institution . ' AND M.`class`=' . $class . ' AND M.`section`=' . $section . ' AND M.`group`=' . $group . ' AND M.`exam`=' . $exam . ' AND M.`academic_year`=' . $year . ' AND M.`failed_subject`=0 ORDER BY M.`group_position` ASC');
        ...
        foreach ($students as $key => $values) {
            ...
            <tr>
                ...
                <td class="text-center"><?= $values['group_position'] ?></td>
            </tr> 
            <?php $i++;
        }
        ?>   
