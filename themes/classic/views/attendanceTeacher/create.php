...
$this->pageTitle = 'New Teacher Attendance';
$this->breadcrumbs = array(
    'Attendance Teachers' => array('admin'),
    'Create',
);
?>
<script>
                    ...
                    <div class="widget-body no-padding">                        
                        ...
                        <table class="table table-bordered table-condensed table-responsive">
                            ...
                            <tbody>                                
                                <?php
                                $i = 1;
                                $array = Teacher::model()->findAll(array('condition' => 'institution=' . Yii::app()->user->institution . ' AND status="Active"', 'order' => 'teacher_name'));
                                foreach ($array as $key => $value) {
                                    echo $form->hiddenField($model, "[$i]teacherid", array('value' => $value['id']));
                                    echo '<tr>';
                                    echo '<td class="text-center width-50">' . $i . '</td>';
                                    echo '<td class="text-center width-50">' . Teacher::getPhoto($value['id']) . '</td>';
                                    echo '<td class="text-left">' . $value['teacher_name'] . '</td>';
                                    echo '<td class="text-left">' . $value['designation'] . '</td>';
                        ...
                        <footer>
                            <?php echo CHtml::submitButton('PROCESS TEACHER ATTENDANCE', array('class' => 'btn btn-primary')); ?>
                        </footer>
                        <?php $this->endWidget(); ?>
                    </div>
                    <!-- end widget content -->
