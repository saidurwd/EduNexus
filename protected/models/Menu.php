...
class Menu extends CActiveRecord {
    ...
    public static function get_menu() {
        $parent1 = Yii::app()->db->createCommand()
                ->join('{{menu_access}} ma', 't.id=ma.menu')
                ->select('t.id, t.parent, t.title, t.url, t.icon, t.ordering, t.status, t.group')
                ->from('{{menu}} t')
                ->where('t.parent=0 AND t.status=1 AND ma.`group`=' . Yii::app()->user->group)
                ->order('t.ordering ASC, t.title ASC')
                ->queryAll();
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            $parent2 = Yii::app()->db->createCommand()
                    ...
                    ->select('t.id, t.parent, t.title, t.url, t.icon, t.ordering, t.status, t.group')
                    ->from('{{menu}} t')
                    ->where('t.parent=' . $values1["id"] . ' AND t.status=1 AND ma.`group`=' . Yii::app()->user->group)
                    ->order('t.ordering ASC, t.title ASC')
                    ->queryAll();
            if (count($parent2) > 0) {
                echo '<ul>';
                foreach ($parent2 as $key => $values2) {
                    ...
                    $parent3 = Yii::app()->db->createCommand()
                            ...
                            ->select('t.id, t.parent, t.title, t.url, t.icon, t.ordering, t.status, t.group')
                            ->from('{{menu}} t')
                            ->where('t.parent=' . $values2["id"] . ' AND t.status=1 AND ma.`group`=' . Yii::app()->user->group)
                            ->order('t.ordering ASC, t.title ASC')
                            ->queryAll();
                    if (count($parent3) > 0) {
                        echo '<ul>';
