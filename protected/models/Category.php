...
class Category extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('id, institution, parent, title, alias, details, path, status, created_by, created_on', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function relations() {
        ...
        return array(
            'contents' => array(self::HAS_MANY, 'Content', 'category'),
        );
    }
    ...
    public static function update_path($id) {
        ...
        if ($model->parent == 0 || $model->parent === null) {
            ...
            $parent = Category::model()->findByAttributes(array('id' => $model->parent));
    ...
    public static function get_full_path($id) {
        ...
        foreach ($array as $key => $value) {
            if ($value > 0) {
                if ($total != $i) {
                    $data .= Category::getData($value, 'title') . ' <i class="fa fa-angle-double-right text-info"></i> ';
                } else {
                    $data .= Category::getData($value, 'title');
                }
            }
            $i++;
        }
    ...
    public static function getCategory($model, $field, $id) {
        $parent1 = Category::model()->findAll(array('condition' => '(parent=0 OR parent IS NULL) AND (institution=0 OR institution=' . Yii::app()->user->institution . ')', 'order' => "title"));
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            $parent2 = Category::model()->findAll(array('condition' => 'parent=' . (int) $values1["id"], 'order' => 'title'));
    ...
    public static function getCategoryForm($model, $field, $id) {
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            foreach ($parent2 as $key => $values2) {
                ...
                $parent3 = Category::model()->findAll(array('condition' => 'parent=' . (int) $values2["id"], 'order' => 'path'));
                foreach ($parent3 as $key => $values3) {
                    ...
                    $parent4 = Category::model()->findAll(array('condition' => 'parent=' . (int) $values3["id"], 'order' => 'path'));
