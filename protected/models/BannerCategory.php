...
class BannerCategory extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('id, institution, parent, title, details, alias, path, status, created_by, created_on', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function relations() {
        ...
        return array(
            'banners' => array(self::HAS_MANY, 'Banner', 'category'),
        );
    }
    ...
    public function search() {
        ...
        return new CActiveDataProvider($this, array(
            ...
            'sort' => array('defaultOrder' => 'path DESC')
        ));
    }
    ...
    public static function getData($id, $field) {
        $model = BannerCategory::model()->findByAttributes(array('id' => $id));
    ...
    public static function update_path($id) {
        ...
        if ($model->parent == 0 || $model->parent === null) {
            ...
            $parent = BannerCategory::model()->findByAttributes(array('id' => $model->parent));
    ...
    public static function get_full_path($id) {
        ...
        foreach ($array as $key => $value) {
            if ($value > 0) {
                if ($total != $i) {
                    $data .= BannerCategory::getData($value, 'title') . ' <i class="fa fa-angle-double-right text-info"></i> ';
                } else {
                    $data .= BannerCategory::getData($value, 'title');
                }
            }
            $i++;
        }
    ...
    public static function getCategory($model, $field, $id) {
        $parent1 = BannerCategory::model()->findAll(array('condition' => '(parent=0 OR parent IS NULL) AND (institution=0 OR institution=' . Yii::app()->user->institution . ')', 'order' => "title"));
        $option = '<select id="' . $model . '_' . $field . '" name="' . $model . '[' . $field . ']" class="select2">';
        $option .= '<option value="">Select a BannerCategory</option>';
        foreach ($parent1 as $key => $values1) {
            $option .= '<optgroup label="' . $values1["title"] . '">';
            $parent2 = BannerCategory::model()->findAll(array('condition' => 'parent=' . (int) $values1["id"], 'order' => 'title'));
    ...
    public static function getCategoryForm($model, $field, $id) {
        $parent1 = BannerCategory::model()->findAll(array('condition' => '(parent=0 OR parent IS NULL) AND (institution=0 OR institution=' . Yii::app()->user->institution . ')', "order" => "path"));
        $option = '<select id="' . $model . '_' . $field . '" name="' . $model . '[' . $field . ']" class="select2">';
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            $parent2 = BannerCategory::model()->findAll(array('condition' => 'parent=' . (int) $values1["id"], 'order' => 'path'));
            foreach ($parent2 as $key => $values2) {
                if ($id == $values2["id"]) {
                ...
                $parent3 = BannerCategory::model()->findAll(array('condition' => 'parent=' . (int) $values2["id"], 'order' => 'path'));
                foreach ($parent3 as $key => $values3) {
                    ...
                    $parent4 = BannerCategory::model()->findAll(array('condition' => 'parent=' . (int) $values3["id"], 'order' => 'path'));
