...
class DocumentCategory extends CActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            'documents' => array(self::HAS_MANY, 'Document', 'cetegory'),
        );
    }
    ...
    public static function getData($id, $field) {
        $model = DocumentCategory::model()->findByAttributes(array('id' => $id));
    ...
    public static function update_path($id) {
        ...
        if ($model->parent == 0 || $model->parent === null) {
            ...
            $parent = DocumentCategory::model()->findByAttributes(array('id' => $model->parent));
    ...
    public static function get_full_path($id) {
        ...
        foreach ($array as $key => $value) {
            if ($value > 0) {
                if ($total != $i) {
                    $data .= DocumentCategory::getData($value, 'title') . ' <i class="fa fa-angle-double-right text-info"></i> ';
                } else {
                    $data .= DocumentCategory::getData($value, 'title');
                }
            }
            $i++;
        }
    ...
    public static function getCategory($model, $field, $id) {
        $parent1 = DocumentCategory::model()->findAll(array('condition' => '(parent=0 OR parent IS NULL) AND (institution=0 OR institution=' . Yii::app()->user->institution . ')', 'order' => "title"));
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            $parent2 = DocumentCategory::model()->findAll(array('condition' => 'parent=' . (int) $values1["id"], 'order' => 'title'));
    ...
    public static function getCategoryForm($model, $field, $id) {
        ...
        foreach ($parent1 as $key => $values1) {
            ...
            foreach ($parent2 as $key => $values2) {
                ...
                $parent3 = DocumentCategory::model()->findAll(array('condition' => 'parent=' . (int) $values2["id"], 'order' => 'path'));
                foreach ($parent3 as $key => $values3) {
                    ...
                    $parent4 = DocumentCategory::model()->findAll(array('condition' => 'parent=' . (int) $values3["id"], 'order' => 'path'));
