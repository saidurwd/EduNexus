...
class AssignmentController extends Controller {
    ...
    public function actionCreate() {
        ...
        $path = Yii::app()->basePath . '/../uploads/assignment/' . Yii::app()->user->institution;
        if (!is_dir($path)) {
        ...
        if (isset($_POST['Assignment'])) {
            ...
            if ($model->validate()) {
                //File upload script
                if (@!empty($_FILES['Assignment']['name']['file'])) {
                    $model->file = $_POST['Assignment']['file'];
                    if ($model->validate(array('file'))) {
                        $model->file = CUploadedFile::getInstance($model, 'file');
                    } else {
                        $model->file = '';
                    }
                    $model->file->saveAs($path . '/' . time() . '_' . str_replace(' ', '_', strtolower($model->file)));
                    $model->file = time() . '_' . str_replace(' ', '_', strtolower($model->file));
                }
                if ($model->save()) {
...
//        $model = $this->loadModel($id);
//        $myFile = Yii::app()->basePath . '/../uploads/assignment/' . Yii::app()->user->institution . '/' . $model->file;
//        if ((is_file($myFile)) && (file_exists($myFile))) {
//            unlink($myFile);
//        }        
