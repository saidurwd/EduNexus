...
class WeblinkController extends Controller {
    ...
    public function actionCreate() {
        $model = new Weblink;
        $path = Yii::app()->basePath . '/../uploads/weblink/' . Yii::app()->user->institution;
        if (!is_dir($path)) {
        ...
        if (isset($_POST['Weblink'])) {
            ...
            if ($model->validate()) {
                //Picture upload
                if (@!empty($_FILES['Weblink']['name']['picture'])) {
                    $model->picture = $_POST['Weblink']['picture'];
            ...
            if ($model->validate()) {
                ...
                if ($model->save()) {
                    if (empty($model->picture)) {
                        Content::model()->updateAll(array('picture' => $previuosFileName), 'id=' . (int) $model->id);
                    }
                    Yii::app()->user->setFlash('success', 'Data was saved successfully');
                    $this->redirect(array('admin'));
                }
