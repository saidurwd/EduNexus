...
class ContentController extends Controller {
    ...
    public function actionCreate() {
        $model = new Content;
        $path = Yii::app()->basePath . '/../uploads/content/' . Yii::app()->user->institution;
        if (!is_dir($path)) {
        ...
        if (isset($_POST['Content'])) {
            ...
            if ($model->validate()) {
                ...
                if (@!empty($_FILES['Content']['name']['image'])) {
                    $model->image = $_POST['Content']['image'];
                    if ($model->validate(array('image'))) {
                        $model->image = CUploadedFile::getInstance($model, 'image');
                    } else {
                        $model->image = null;
                    }
                    $model->image->saveAs($path . '/' . time() . '_' . str_replace(' ', '_', strtolower($model->image)));
                }
                $model->image = time() . '_' . str_replace(' ', '_', strtolower($model->image));
                if ($model->save()) {
        ...
        if (isset($_POST['Content'])) {
            ...
            if ($model->validate()) {
                if (@!empty($_FILES['Content']['name']['image'])) {
                    ...
                    $model->image = $previuosFileName;
                }
                if ($model->save()) {
                    if (empty($model->image)) {
                        Content::model()->updateAll(array('image' => $previuosFileName), 'id=' . (int) $model->id);
                    }
                    Yii::app()->user->setFlash('success', 'Data was saved successfully');
                    $this->redirect(array('admin'));
                }
